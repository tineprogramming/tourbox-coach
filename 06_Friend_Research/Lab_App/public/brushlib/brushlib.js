...
exports.create = function create(canvas) {
  ...
  return Module().then((Module) => {
    ...
    function getColorProxy(x, y, radius, rPointer, gPointer, bPointer, aPointer) {
      // DrawCopilotLab patch (2026-05-10):
      //  1. Signature changed from 'ifffiiii' (return int) to 'vfffiiii'
      //     (return void). libmypaint's `MyPaintSurfaceGetColorFunction`
      //     returns void; the original 'i' triggered a "function signature
      //     mismatch" inside the wasm whenever a brush with smudge > 0
      //     (or a smudge mapping that becomes >0 mid-stroke, e.g. via
      //     `smudge.inputs.speed1`) caused libmypaint to actually invoke
      //     this callback. Affected wash, smudge, pastel, charcoal, etc.
      //  2. `Module.setValue` is no longer a default Emscripten export in
      //     newer toolchains; it isn't compiled into this wasm bundle.
      //     We write directly to HEAPF32 instead. `ptr >> 2` converts the
      //     byte address into a float-index (4 bytes per float).

      const result = bindings.getColor(x, y, radius)

      Module.HEAPF32[rPointer >> 2] = result[0]
      Module.HEAPF32[gPointer >> 2] = result[1]
      Module.HEAPF32[bPointer >> 2] = result[2]
      Module.HEAPF32[aPointer >> 2] = result[3]
    }
    ...
    class Painter {
      ...
      constructor(ctx) {
        // DrawCopilotLab patch (2026-05-10): pass `willReadFrequently` so
        // smudge / palette-knife brushes (which call `getImageData` once
        // per dab) get the readback-optimized 2D backend. Without this
        // flag Chrome warns and can fall back to GPU-readback paths that
        // hitch during long smudge strokes.
        this.ctx = isFunction(ctx.getContext) ? ctx.getContext('2d', { willReadFrequently: true }) : ctx
        this.radiusRatio = 1
      }
      ...
      getColor = (x, y, radius) => {
        // DrawCopilotLab patch (2026-05-10):
        // The previous implementation did `pixel[0] /= 255` on
        // `image.data`, which is a Uint8ClampedArray — that conversion
        // rounds the float result back to an integer, destroying all
        // color information (a 200/255 = 0.78 became 1, etc.). Smudge
        // and palette-knife brushes therefore had nothing useful to pull
        // from the canvas. We copy out into a regular array so the
        // division yields proper floats.
        const ctx = this.ctx
        // Floor coords; getImageData wants integers, and during stroke
        // sub-pixel positions are common.
        const ix = Math.max(0, Math.min(ctx.canvas.width - 1, Math.floor(x)))
        const iy = Math.max(0, Math.min(ctx.canvas.height - 1, Math.floor(y)))
        const image = ctx.getImageData(ix, iy, 1, 1)
        const d = image.data
