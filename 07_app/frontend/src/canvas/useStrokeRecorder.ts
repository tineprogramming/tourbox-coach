...
export function useStrokeRecorder() {
  ...
  const onPointerUp = useCallback(
    (native: PointerEvent, x: number, y: number) => {
      ...
      // Stream the completed stroke to the Pi backend (Pen-Process Recorder
      // = the moat). Queues if disconnected; never blocks the UI.
      const { coachProvider: provider, locale } = useCanvasStore.getState();
      wsClient.send({ type: "stroke_complete", stroke: finished, provider, lang: locale });
    },
