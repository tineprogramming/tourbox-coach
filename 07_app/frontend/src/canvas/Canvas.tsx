...
export function Canvas() {
  ...
  const ghostLesson = useCanvasStore((s) => s.ghostLesson);
  const ghostOpacity = useCanvasStore((s) => s.ghostOpacity);

  // ---- Ghost Guide reference image loader ----
  // Loads the active lesson image and exposes it as an HTMLImageElement that
  // Konva.Image can render. Null when no lesson is picked or while loading.
  const [referenceImage, setReferenceImage] = useState<HTMLImageElement | null>(null);
  useEffect(() => {
    setReferenceImage(null);
    if (!ghostLesson) return;
    const lesson = LESSONS.find((L) => L.id === ghostLesson);
    if (!lesson) return;
    const img = new Image();
  ...
  // Compute "contain" geometry so the reference fits centered without
  // cropping or distortion.
  const refGeom = useMemo(() => {
    if (!referenceImage || size.width === 0 || size.height === 0) return null;
    const iw = referenceImage.naturalWidth || referenceImage.width;
    const ih = referenceImage.naturalHeight || referenceImage.height;
    if (!iw || !ih) return null;
    const scale = Math.min(size.width / iw, size.height / ih);
    const w = iw * scale;
    const h = ih * scale;
    return { x: (size.width - w) / 2, y: (size.height - h) / 2, w, h };
  }, [referenceImage, size.width, size.height]);
  ...
  return (
    <div ref={containerRef} className="canvas-host">
      ...
      >
        {/* Ghost Guide reference (the "magic dial"). Bottom-most layer so the
            user's marks naturally cover it as they paint over the reference.
            Opacity is the hero interaction — eventually wired to the TourBox
            dial via Web HID. */}
        <Layer listening={false} opacity={ghostOpacity} visible={!!referenceImage && !!refGeom}>
          {referenceImage && refGeom && (
            <KonvaImage
